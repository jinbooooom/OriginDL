...
public:
     ...
     * 默认实现：对于单shape算子（get_required_shapes_count() == 1），返回 {1,1}, {10,10}, {100,100}, {1000,1000}, {10000,10000}
     * 多shape算子需要重写此方法
     */
    ...
    /**
     * @brief 将DeviceType转换为简单的设备字符串 "cpu" 或 "cuda"
     * @param device_type 设备类型
     * @return 设备字符串
     */
    static std::string device_type_to_string(DeviceType device_type);
...
private:
     ...
     */
    void process_devices(std::vector<DeviceType>& devices, bool use_default_devices) const;
};
