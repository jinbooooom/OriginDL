...
class IdentityBenchmark(BenchmarkFramework):
    """Identity算子基准测试类"""
    
    def get_required_shapes_count(self) -> int:
        """Identity算子只需要一个shape"""
        return 1
    
    def run_benchmark(self, config: BenchmarkConfig) -> float:
        """执行PyTorch的identity操作性能测试"""
        if len(config.shapes) != 1:
            raise ValueError(f"Identity benchmark requires exactly 1 shape, got {len(config.shapes)}")
        ...
        # Identity就是直接返回输入，PyTorch没有专门的identity函数，使用clone模拟
        # 预热
        for _ in range(config.warmup_cnt):
            result = x.clone()  # 使用clone模拟identity
        if device.type == 'cuda':
    ...
    def validate_shapes(self, shapes: List[Tuple[int, ...]]) -> None:
        """验证形状是否有效"""
        if len(shapes) != 1:
            raise ValueError(f"Identity requires exactly 1 shape, got {len(shapes)}")


def benchmark_identity_comparison(device_filter=None,
                             shape_filter=None,
                             warmup_cnt=5,
                             repeat_cnt=100,
                             inplace=False,
    ...
    Args:
        ...
        warmup_cnt: 预热次数
        repeat_cnt: 重复次数
        inplace: 是否使用就地操作（identity不支持inplace）
        verbose: 是否输出详细信息
    ...
    return benchmark.run(
        ...
        inplace=False,  # identity不支持inplace
        verbose=verbose
    )
