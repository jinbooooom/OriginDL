...
class SiluBenchmark(BenchmarkFramework):
    """SiLU算子基准测试类"""
    ...
    def run_benchmark(self, config: BenchmarkConfig) -> float:
        """执行PyTorch的silu操作性能测试"""
        if len(config.shapes) != 1:
            raise ValueError(f"SiLU benchmark requires exactly 1 shape, got {len(config.shapes)}")
    ...
    def validate_shapes(self, shapes: List[Tuple[int, ...]]) -> None:
        """验证形状是否有效"""
        if len(shapes) != 1:
            raise ValueError(f"SiLU requires exactly 1 shape, got {len(shapes)}")


def benchmark_silu_comparison(device_filter=None,
                             shape_filter=None,
                             warmup_cnt=5,
                             repeat_cnt=100,
                             inplace=False,
    ...
    Args:
        ...
        warmup_cnt: 预热次数
        repeat_cnt: 重复次数
        inplace: 是否使用就地操作（silu不支持inplace）
        verbose: 是否输出详细信息
    
    Returns:
        PyTorch测试结果列表
    """
    benchmark = SiluBenchmark()
    return benchmark.run(
        device_filter=device_filter,
        shape_filter=shape_filter,
        warmup_cnt=warmup_cnt,
        repeat_cnt=repeat_cnt,
        inplace=False,  # silu不支持inplace
        verbose=verbose
    )
