...
class SoftmaxBenchmark(BenchmarkFramework):
    """Softmax算子基准测试类"""
    ...
    def run_benchmark(self, config: BenchmarkConfig) -> float:
        """执行PyTorch的softmax操作性能测试"""
        if len(config.shapes) != 1:
            raise ValueError(f"Softmax benchmark requires exactly 1 shape, got {len(config.shapes)}")
    ...
    def validate_shapes(self, shapes: List[Tuple[int, ...]]) -> None:
        """验证形状是否有效"""
        if len(shapes) != 1:
            raise ValueError(f"Softmax requires exactly 1 shape, got {len(shapes)}")


def benchmark_softmax_comparison(device_filter=None,
                             shape_filter=None,
                             warmup_cnt=5,
                             repeat_cnt=100,
                             inplace=False,
    ...
    Args:
        ...
        warmup_cnt: 预热次数
        repeat_cnt: 重复次数
        inplace: 是否使用就地操作（softmax不支持inplace）
        verbose: 是否输出详细信息
    ...
    return benchmark.run(
        ...
        inplace=False,  # softmax不支持inplace
        verbose=verbose
    )
