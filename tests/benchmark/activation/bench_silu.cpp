 ...
 * @brief SiLU算子基准测试类
 */
class SiluBenchmark : public BenchmarkFramework
{
public:
    double run_benchmark(const BenchmarkConfig &config) override
    {
        ...
        {
            THROW_RUNTIME_ERROR("SiLU benchmark requires exactly 1 shape, got {}", config.shapes.size());
        }
    ...
    void validate_shapes(const std::vector<Shape> &shapes) const override
    {
        if (shapes.size() != 1)
        {
            THROW_RUNTIME_ERROR("SiLU requires exactly 1 shape, got {}", shapes.size());
        }
    }

    std::string get_operator_name() const override { return "SiLU"; }
};

int main(int argc, char *argv[])
{
    SiluBenchmark benchmark;
