...
class SigmoidBenchmark(BenchmarkFramework):
    """Sigmoid算子基准测试类"""
    ...
    def run_benchmark(self, config: BenchmarkConfig) -> float:
        """执行PyTorch的sigmoid操作性能测试"""
        if len(config.shapes) != 1:
            raise ValueError(f"Sigmoid benchmark requires exactly 1 shape, got {len(config.shapes)}")
        ...
        # Sigmoid不支持inplace操作
        if config.inplace:
            raise ValueError("Sigmoid operator does not support inplace operations")
    ...
    def validate_shapes(self, shapes: List[Tuple[int, ...]]) -> None:
        """验证形状是否有效"""
        if len(shapes) != 1:
            raise ValueError(f"Sigmoid requires exactly 1 shape, got {len(shapes)}")


def benchmark_sigmoid_comparison(device_filter=None,
                             shape_filter=None,
                             warmup_cnt=5,
                             repeat_cnt=100,
                             inplace=False,
