...
class AvgPool2dBenchmark(BenchmarkFramework):
    """AvgPool2d算子基准测试类"""
    
    def get_required_shapes_count(self) -> int:
        """AvgPool2d算子只需要一个shape（4D输入）"""
        return 1
    
    def run_benchmark(self, config: BenchmarkConfig) -> float:
        """执行PyTorch的avg_pool2d操作性能测试"""
        if len(config.shapes) != 1:
            raise ValueError(f"AvgPool2d benchmark requires exactly 1 shape, got {len(config.shapes)}")
        ...
        if len(shape) != 4:
            raise ValueError(f"AvgPool2d requires 4D shape (N, C, H, W), got {shape}")
        ...
        for _ in range(config.warmup_cnt):
            result = F.avg_pool2d(x, kernel_size, stride, pad)
        if device.type == 'cuda':
    ...
    def validate_shapes(self, shapes: List[Tuple[int, ...]]) -> None:
        """验证形状是否有效"""
        if len(shapes) != 1:
            raise ValueError(f"AvgPool2d requires exactly 1 shape, got {len(shapes)}")
        if len(shapes[0]) != 4:
            raise ValueError(f"AvgPool2d requires 4D shape (N, C, H, W), got {shapes[0]}")


def benchmark_avg_pool2d_comparison(device_filter=None,
                             shape_filter=None,
                             warmup_cnt=5,
                             repeat_cnt=100,
                             inplace=False,
    ...
    Args:
        ...
        warmup_cnt: 预热次数
        repeat_cnt: 重复次数
        inplace: 是否使用就地操作（avg_pool2d不支持inplace）
        verbose: 是否输出详细信息
    
    Returns:
        PyTorch测试结果列表
    """
    benchmark = AvgPool2dBenchmark()
    return benchmark.run(
        device_filter=device_filter,
        shape_filter=shape_filter,
        warmup_cnt=warmup_cnt,
        repeat_cnt=repeat_cnt,
        inplace=False,  # avg_pool2d不支持inplace
        verbose=verbose
    )
