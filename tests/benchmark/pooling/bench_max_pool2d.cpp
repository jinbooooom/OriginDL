...
/**
 * @brief MaxPool2d算子基准测试类
 */
class MaxPool2dBenchmark : public BenchmarkFramework
{
public:
    double run_benchmark(const BenchmarkConfig &config) override
    {
        ...
        {
            THROW_RUNTIME_ERROR("MaxPool2d benchmark requires exactly 1 shape, got {}", config.shapes.size());
        }
        ...
        if (shape.ndims() != 4)
        {
            THROW_RUNTIME_ERROR("MaxPool2d requires 4D shape (N, C, H, W), got {}", shape.to_string());
        }
        ...
        {
            auto result = F::max_pool2d(x, kernel_size, stride, pad);
        }
    ...
    void validate_shapes(const std::vector<Shape> &shapes) const override
    {
        if (shapes.size() != 1)
        {
            THROW_RUNTIME_ERROR("MaxPool2d requires exactly 1 shape, got {}", shapes.size());
        }
        if (shapes[0].ndims() != 4)
        {
            THROW_RUNTIME_ERROR("MaxPool2d requires 4D shape (N, C, H, W), got {}", shapes[0].to_string());
        }
    }

    std::string get_operator_name() const override { return "MaxPool2d"; }
};

int main(int argc, char *argv[])
{
    MaxPool2dBenchmark benchmark;
