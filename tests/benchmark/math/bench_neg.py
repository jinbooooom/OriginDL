...
class NegBenchmark(BenchmarkFramework):
    """Neg算子基准测试类"""
    ...
    def run_benchmark(self, config: BenchmarkConfig) -> float:
        ...
        if len(config.shapes) != 1:
            raise ValueError(f"Neg benchmark requires exactly 1 shape, got {len(config.shapes)}")
    ...
    def validate_shapes(self, shapes: List[Tuple[int, ...]]) -> None:
        """验证形状是否有效"""
        if len(shapes) != 1:
            raise ValueError(f"Neg requires exactly 1 shape, got {len(shapes)}")


def benchmark_neg_comparison(device_filter=None,
                             shape_filter=None,
                             warmup_cnt=5,
                             repeat_cnt=100,
                             inplace=False,
