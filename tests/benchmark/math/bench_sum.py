...
class SumBenchmark(BenchmarkFramework):
    """Sum算子基准测试类"""
    ...
    def run_benchmark(self, config: BenchmarkConfig) -> float:
        ...
        if len(config.shapes) != 1:
            raise ValueError(f"Sum benchmark requires exactly 1 shape, got {len(config.shapes)}")
        ...
        # 在最后一个维度上求和
        dim = len(shape) - 1 if len(shape) > 0 else None
        
        # 预热
        for _ in range(config.warmup_cnt):
            if dim is not None:
                result = torch.sum(x, dim=dim, keepdim=False)
            else:
                result = torch.sum(x)
        if device.type == 'cuda':
    ...
    def validate_shapes(self, shapes: List[Tuple[int, ...]]) -> None:
        """验证形状是否有效"""
        if len(shapes) != 1:
            raise ValueError(f"Sum requires exactly 1 shape, got {len(shapes)}")


def benchmark_sum_comparison(device_filter=None,
                             shape_filter=None,
                             warmup_cnt=5,
                             repeat_cnt=100,
                             inplace=False,
    ...
    Args:
        ...
        warmup_cnt: 预热次数
        repeat_cnt: 重复次数
        inplace: 是否使用就地操作（sum不支持inplace）
        verbose: 是否输出详细信息
    
    Returns:
        PyTorch测试结果列表
    """
    benchmark = SumBenchmark()
    return benchmark.run(
        device_filter=device_filter,
        shape_filter=shape_filter,
        warmup_cnt=warmup_cnt,
        repeat_cnt=repeat_cnt,
        inplace=False,  # sum不支持inplace
        verbose=verbose
    )
