...
/**
 * @brief Flatten算子基准测试类
 */
class FlattenBenchmark : public BenchmarkFramework
{
public:
    double run_benchmark(const BenchmarkConfig &config) override
    {
        ...
        {
            THROW_RUNTIME_ERROR("Flatten benchmark requires exactly 1 shape, got {}", config.shapes.size());
        }
        ...
        if (shape.ndims() < 2)
        {
            THROW_RUNTIME_ERROR("Flatten requires at least 2D shape, got {}", shape.to_string());
        }
    ...
    void validate_shapes(const std::vector<Shape> &shapes) const override
    {
        if (shapes.size() != 1)
        {
            THROW_RUNTIME_ERROR("Flatten requires exactly 1 shape, got {}", shapes.size());
        }
        if (shapes[0].ndims() < 2)
        {
            THROW_RUNTIME_ERROR("Flatten requires at least 2D shape, got {}", shapes[0].to_string());
        }
    }

    std::string get_operator_name() const override { return "Flatten"; }
};

int main(int argc, char *argv[])
{
    FlattenBenchmark benchmark;
