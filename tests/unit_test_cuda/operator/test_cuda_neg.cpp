...
{
    // 测试运算符重载
    auto x = Tensor({2.0, -3.0}, Shape{2}, dtype(Float32).device(kCUDA));
...
{
    // 测试零张量
    auto x = Tensor({0.0, 0.0}, Shape{2}, dtype(Float32).device(kCUDA));
...
TEST_F(NegOperatorTest, BackwardWithGradient)
{
    ...
    // 设置输出梯度为2
    auto gy = Tensor({2.0, 2.0}, Shape{2}, dtype(Float32).device(kCUDA));
    y.backward();
...
{
    // 测试数值稳定性
    auto x = Tensor({1e10, 1e-10}, Shape{2}, dtype(Float32).device(kCUDA));
...
{
    // 测试精度
    auto x = Tensor({0.1, 0.2}, Shape{2}, dtype(Float32).device(kCUDA));
...
{
    // 测试混合符号
    auto x = Tensor({1.0, -2.0, 0.0, -4.0, 5.0}, Shape{5}, dtype(Float32).device(kCUDA));
