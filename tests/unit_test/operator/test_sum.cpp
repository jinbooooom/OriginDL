...
TEST_P(SumOperatorTest, ThreeDimensionalWithAxis)
{
    ...
    auto expected0 = Tensor({40.0f, 44.0f, 48.0f, 52.0f, 56.0f, 60.0f}, expected_shape0, dtype(DataType::kFloat32).device(deviceType()));
    origin::test::GTestUtils::EXPECT_TENSORS_EQ(result0, expected0, origin::test::TestTolerance::kDefault);
    ...
    auto expected1 = Tensor({9.0f, 12.0f, 27.0f, 30.0f, 45.0f, 48.0f, 63.0f, 66.0f}, expected_shape1, dtype(DataType::kFloat32).device(deviceType()));
    origin::test::GTestUtils::EXPECT_TENSORS_EQ(result1, expected1, origin::test::TestTolerance::kDefault);
    ...
    auto expected2 = Tensor({3.0f, 7.0f, 11.0f, 15.0f, 19.0f, 23.0f, 27.0f, 31.0f, 35.0f, 39.0f, 43.0f, 47.0f}, expected_shape2, dtype(DataType::kFloat32).device(deviceType()));
...
TEST_P(SumOperatorTest, AssociativeProperty)
{
    ...
    EXPECT_NEAR(sum_abc.item<float>(), sum_a_plus_sum_b_plus_sum_c.item<float>(), origin::test::TestTolerance::kDefault);
}
