...
TEST_P(SumToOperatorTest, ForwardBasic)
{
    ...
    auto result = F::sum_to(x, target_shape);
...
TEST_P(SumToOperatorTest, ForwardToLargerShape)
{
    ...
    EXPECT_THROW(F::sum_to(x, target_shape), std::runtime_error);
}
...
{
    // 测试恒等性质：F::sum_to(x, x.shape()) = x
    auto x = Tensor({1.0f, 2.0f, 3.0f, 4.0f}, Shape{2, 2}, dtype(DataType::kFloat32).device(deviceType()));

    auto result = F::sum_to(x, x.shape());
...
{
    // 测试结合性质：F::sum_to(F::sum_to(x, shape1), shape2) = F::sum_to(x, shape2)
    auto x = Tensor({1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f}, Shape{2, 3}, dtype(DataType::kFloat32).device(deviceType()));
    Shape shape1{2, 1};
    Shape shape2{1, 1};

    auto result1 = F::sum_to(F::sum_to(x, shape1), shape2);
    auto result2 = F::sum_to(x, shape2);
...
{
    // 测试交换性质：F::sum_to(x + y, shape) = F::sum_to(x, shape) + F::sum_to(y, shape)
    ...
    auto result1 = F::sum_to(x + y, target_shape);
    auto result2 = F::sum_to(x, target_shape) + F::sum_to(y, target_shape);
