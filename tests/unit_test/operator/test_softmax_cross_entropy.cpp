...
{
    // 测试基本反向传播
    auto x = Tensor({1.0f, 2.0f, 3.0f}, Shape{1, 3}, dtype(DataType::kFloat32).device(deviceType()).requires_grad(true));
    auto target = Tensor({2}, Shape{1}, dtype(DataType::kInt32).device(deviceType()));
...
{
    // 测试批处理的反向传播
    auto x = Tensor({1.0f, 2.0f, 3.0f, 3.0f, 2.0f, 1.0f}, Shape{2, 3}, dtype(DataType::kFloat32).device(deviceType()).requires_grad(true));
    auto target = Tensor({2, 0}, Shape{2}, dtype(DataType::kInt32).device(deviceType()));
