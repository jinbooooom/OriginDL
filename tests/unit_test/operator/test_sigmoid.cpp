...
{
    // 测试基本反向传播
    // sigmoid'(x) = F::sigmoid(x) * (1 - F::sigmoid(x))
...
{
    // 测试范围性质：F::sigmoid(x) 应该在 [0, 1] 范围内
    ...
    // 对于非极值，应该在 (0, 1) 范围内
    // result_data[0] 是 F::sigmoid(-100)，可能接近0，允许等于0
    // result_data[1] 是 F::sigmoid(-10)，应该在 (0, 1) 范围内
    EXPECT_GT(result_data[1], 0.0f);  // F::sigmoid(-10) > 0
    EXPECT_LT(result_data[1], 1.0f);  // F::sigmoid(-10) < 1
    EXPECT_GT(result_data[2], 0.0f);  // F::sigmoid(0) = 0.5 > 0
    EXPECT_LT(result_data[2], 1.0f);  // F::sigmoid(0) = 0.5 < 1
    EXPECT_GT(result_data[3], 0.0f);  // F::sigmoid(10) > 0
    EXPECT_LT(result_data[3], 1.0f);  // F::sigmoid(10) < 1
    // result_data[4] 是 F::sigmoid(100)，可能接近1，允许等于1
}

TEST_P(SigmoidOperatorTest, SymmetryProperty)
{
    // 测试对称性质：F::sigmoid(-x) = 1 - F::sigmoid(x)
    ...
    auto sigmoid_x     = F::sigmoid(x);
    auto sigmoid_neg_x = F::sigmoid(neg_x);
