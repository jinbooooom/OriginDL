...
用法示例：
./build/bin/example/example_yolov5 -i data/imgs/ -o data/outputs/ -p model/yolo/yolov5n_small.pnnx.param -b
model/yolo/yolov5n_small.pnnx.bin
*/
#include <getopt.h>
#include <algorithm>
#include <cmath>
            ...
            if (shape_end != std::string::npos)
            {
                ...
                if (dims.size() >= 4)
                {
                    try
                    {
                        ...
                        logi("Parsed input shape from param file: batch_size={}, input_h={}, input_w={}", batch_size,
                             input_h, input_w);
                        return true;
                    }
...
UserCfg parse_args(int argc, char *argv[])
{
    ...
    static struct option long_options[] = {{"param", required_argument, 0, 'p'},
...
Tensor create_test_input(const Device &device, int batch_size = 1, int channels = 3, int height = 640, int width = 640)
{
    ...
    Shape input_shape{static_cast<size_t>(batch_size), static_cast<size_t>(channels), static_cast<size_t>(height),
                                ...
                                const std::vector<std::string> &class_names)
{
    ...
    logi("Image {}: Before NMS: {} boxes (confidence >= {})", std::filesystem::path(image_path).filename().string(),
         boxes.size(), confidence_thresh);

    // NMS
    std::vector<int> indices;
    cv::dnn::NMSBoxes(boxes, confs, confidence_thresh, iou_thresh, indices);

    logi("Image {}: After NMS: {} boxes", std::filesystem::path(image_path).filename().string(), indices.size());
    ...
    for (int idx : indices)
    {
        ...
        scale_coords(cv::Size{input_w, input_h}, det.box, cv::Size{origin_input_w, origin_input_h});
        det.conf     = confs[idx];
    ...
    for (const auto &detection : detections)
    {
        ...
        {
            label = class_names[detection.class_id] + " " + std::to_string(detection.conf).substr(0, 4);
        }
        else
        {
            label = "Class " + std::to_string(detection.class_id) + " (" + std::to_string(detection.conf).substr(0, 4) +
                    ")";
        }
        ...
        // 绘制文字背景框（半透明背景）
        cv::Rect text_bg_rect(text_x - text_padding, text_y - text_size.height - text_padding,
                              text_size.width + 2 * text_padding, text_size.height + baseline + 2 * text_padding);

        // 确保背景框在检测框内
        text_bg_rect.x      = std::max(text_bg_rect.x, detection.box.x);
        text_bg_rect.y      = std::max(text_bg_rect.y, detection.box.y);
        text_bg_rect.width  = std::min(text_bg_rect.width, detection.box.x + detection.box.width - text_bg_rect.x);
        text_bg_rect.height = std::min(text_bg_rect.height, detection.box.y + detection.box.height - text_bg_rect.y);
        ...
        cv::putText(result_image, label, cv::Point(text_x, text_y), font_face, current_font_scale, box_color,
                    thickness);
...
int main(int argc, char *argv[])
{
    ...
    if (!get_input_shape_from_param_file(cfg.param_path, parsed_batch_size, cfg.input_h, cfg.input_w))
    {
        cfg.input_h = 640;
        cfg.input_w = 640;
        logw("Failed to parse input shape from param file: {}, using default values: input_h={}, input_w={}",
             cfg.param_path, cfg.input_h, cfg.input_w);
    }
