...
int main()
{
    ...
    float val1      = 4.0f;
    Shape shape     = {2, 2};
    auto x0         = Tensor(val0, shape, requires_grad(true));
    auto x1         = Tensor(val1, shape, requires_grad(true));
    auto y          = -x0;
    auto clear_grad = [&]() {
        y.clear_grad();
        x0.clear_grad();
    ...
    auto a = Tensor({1, 2, 3, 4}, Shape{2, 2}, requires_grad(true));
    auto b = Tensor({5, 6, 7, 8}, Shape{2, 2}, requires_grad(true));
