...
- 输入：`x = [[1, 2], [3, 4]]`，形状 `(2, 2)`
- 丢弃概率：`p = 0.5`
- 缩放因子：`scale = 1/(1-p) = 1/(1-0.5) = 2`
- 假设掩码：`mask = [[2, 0], [2, 2]]`（随机生成，保存缩放因子 `scale` 或 `0`）
  - 位置 `(0,0)` 和 `(1,0)`、`(1,1)` 未被丢弃，`mask` 保存 `scale = 2`
  - 位置 `(0,1)` 被丢弃，`mask` 保存 `0`
- 输出：`y[i] = x[i] * mask[i]`
  - `y = [[1*2, 2*0], [3*2, 4*2]] = [[2, 0], [6, 8]]`，形状 `(2, 2)`

**反向传播：**
- 输出梯度：`gy = [[0.1, 0.2], [0.3, 0.4]]`，形状 `(2, 2)`
- 输入梯度：`gx[i] = gy[i] * mask[i]`
  - `gx = [[0.1*2, 0.2*0], [0.3*2, 0.4*2]] = [[0.2, 0], [0.6, 0.8]]`，形状 `(2, 2)`

**关键说明：**
- `mask` 既用于决定哪些位置传递梯度（`mask = 0` 的位置梯度为 0），也用于保持缩放因子（`mask = scale` 的位置梯度乘以 `scale`）
- 前向传播时：`y[i] = x[i] * mask[i]`，其中 `mask[i] = scale`（未被丢弃）或 `mask[i] = 0`（被丢弃）
- 反向传播时：`gx[i] = gy[i] * mask[i]`，梯度传递与缩放因子保持一致

### 6.2 上采样算子（Upsample）

**当前实现约束：** 仅支持 **4D** 输入，形状为 `(N, C, H, W)`；不支持 3D/5D。**mode** 接口取值 `"nearest"` 或 `"bilinear"`，但当前实现未使用，实际均为最近邻插值。
