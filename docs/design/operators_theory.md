...
求和算子的前向传播执行：`y = sum(x, axis=axis)`

**数学描述：**
- 输入：张量 `x` 和指定的轴 `axis`
- 输出：沿指定轴求和后的张量
- 如果 `axis = -1` 或未指定，则对所有元素求和，输出标量
- 如果指定了 `axis`，则沿该轴求和，输出维度减少 1

**数学表示：**
- `y = sum(x, axis=k)`：沿第 `k` 维求和
- `y[i0, ..., ik-1, ik+1, ...] = Σ(ik) x[i0, ..., ik, ..., in]`

#### 1.11.2 反向传播原理

对于求和运算 `y = sum(x, axis=k)`，根据链式法则和偏导数：
- `∂y/∂x = 1`（在求和维度上）
...
- 沿轴 0 求和：`y = sum(x, axis=0) = [1+4, 2+5, 3+6] = [5, 7, 9]`，形状 `(3,)`
...
**前向传播：**
- 输入：`x = [[1, 2, 3], [4, 5, 6]]`，形状 `(2, 3)`
- 全局求和：`y = sum(x) = 1+2+3+4+5+6 = 21`，形状 `()`
  - 所有维度都被压缩，输出标量
...
压缩到指定形状算子的前向传播执行：`y = sum_to(x, shape)`

**数学描述：**
- 输入：张量 `x` 和目标形状 `shape`
- 输出：将 `x` 压缩到目标形状 `shape`，通过在多余维度上求和
- 压缩规则：在需要减少的维度上求和，保留目标形状中的维度

**数学表示：**
- `y = sum_to(x, shape)`
- 输出 `y` 的形状为 `shape`，数据是 `x` 在压缩维度上的求和

#### 1.13.2 反向传播原理

对于压缩运算 `y = sum_to(x, shape)`，根据链式法则：
- 反向传播是广播：`gx = broadcast_to(gy, x.shape)`

**数学原理：**
- 压缩的反向传播是广播：输出梯度被广播回原始形状
- 因为前向传播时，多个输入元素被求和到一个输出位置
- 反向传播时，输出梯度被分配到所有对应的输入位置
...
**前向传播：**
- 输入：`x = [[0.1, 0.2], [0.3, 0.4]]`，形状 `(2, 2)`
- 目标形状：`(2,)`
- 输出：`y = sum_to(x, (2,)) = [0.1+0.3, 0.2+0.4] = [0.4, 0.6]`，形状 `(2,)`

**反向传播：**
- 输出梯度：`gy = [0.5, 0.6]`，形状 `(2,)`
- 输入梯度：`gx = broadcast_to(gy, (2, 2)) = [[0.5, 0.6], [0.5, 0.6]]`，形状 `(2, 2)`

**数学解释：**
- `y[0] = x[0,0] + x[1,0]`，所以 `gx[0,0] = gy[0] = 0.5`，`gx[1,0] = gy[0] = 0.5`
- `y[1] = x[0,1] + x[1,1]`，所以 `gx[0,1] = gy[1] = 0.6`，`gx[1,1] = gy[1] = 0.6`
...
**数学原理：**
- 平均池化的梯度平均分配到窗口内的所有位置
- 每个输入位置接收的梯度等于输出梯度除以窗口大小
- 这确保了梯度在窗口内的均匀分布

#### 4.2.3 计算示例

##### 示例 4.2.1：平均池化前向与反向传播
...
**数学原理：**
- 自适应平均池化的梯度计算与普通平均池化类似
- 梯度平均分配到窗口内的所有位置
- 窗口大小根据输入和输出大小自适应计算

#### 4.3.3 计算示例

##### 示例 4.3.1：自适应平均池化前向与反向传播
...
- `src/operators/math/`：数学运算算子实现（Add、Sub、Mul、Div、MatMul、Pow、Exp、Log、Neg、Square、Sum、BroadcastTo、SumTo 等）
