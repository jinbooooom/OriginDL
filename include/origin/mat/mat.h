...
public:
     ...
     */
    virtual RMSNormResult rms_norm_forward(const Mat &gamma,
                                           float eps) const = 0;
     ...
     * @return 输出张量，形状与输入相同
     */
    virtual std::unique_ptr<Mat> rms_norm(const Mat &gamma, float eps) const = 0;
     ...
     */
    virtual std::vector<std::unique_ptr<Mat>> rms_norm_backward(const Mat &gy,
                                                                const Mat &gamma,
                                                                const Mat &saved_rms,
                                                                float eps) const = 0;
     ...
     * @param vocab 词表
     * @return token embedding后结果 (, tokens, embedding_dim)
     */
    virtual std::unique_ptr<Mat> embedding(const Mat &vocab) const = 0;
     ...
     * @return 输出梯度
     */
    virtual std::unique_ptr<Mat> embedding_back(const Mat &indices, int vocab_size_, int embedding_dim_) const = 0;
    // === Upsample 相关操作 ===
    /**
