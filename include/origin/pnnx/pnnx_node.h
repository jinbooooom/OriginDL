 ...
 * @details 定义 Parameter（标量/数组等算子参数）、Attribute（权重 shape+data）、PNNXNode（图中单节点：类型、输入输出名、params、attributes、运行时 input/output_tensors 等）。
 * 由 PNNXParser 填充构建时数据，由 PNNXGraph 在 set_inputs/forward 中填充运行时 tensor。被 pnnx_parser.h、pnnx_graph.h、operator_mapper.h 引用。
 */
 ...
 * 也有其它由 .bin 存储的常量（如 YOLO 的 pnnx_5/strides）。shape 由 parse_attribute 解析，data 由 load_weights 从 .bin 读入。
 */
struct Attribute
{
...
/**
 * @brief PNNX 运行时节点
 * @details 表示计算图中的单个算子节点。构建时：name/type/input_names/output_names、params、attributes、shapes 由 PNNXParser 填充；
 * build() 中通过 OperatorMapper::create_operator 创建 op；topological_sort 设置 execution_order。
...
public:
    ...
    std::shared_ptr<Operator> op;  ///< 对应的 origin Operator，build() 时由 OperatorMapper 创建；pnnx.Input/pnnx.Output 无 op（特殊节点）

    std::vector<std::string> input_names;   ///< 依赖的上游张量名。对于 pnnx.Input 节点，input_count=0，此列表为空；其它节点，input_count>=1，指向上游输出
    std::vector<std::string> output_names; ///< 本节点产生的张量名，供下游 input_names 引用。对于 pnnx.Output，output_count=0，此列表为空
    ...
    std::map<std::string, Tensor> input_tensors;  ///< 运行时：上游传入的 Tensor，key 为 input_names 中的名字。对于 pnnx.Input，此映射为空；对于 pnnx.Output，由 propagate_outputs 填充
    std::vector<Tensor> output_tensors;           ///< 运行时：本节点 op 的输出，由 propagate_outputs 传给下游。对于 pnnx.Input，由 set_inputs() 直接设置；对于 pnnx.Output，此列表为空（不计算，只收集）
