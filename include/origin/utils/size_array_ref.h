...
public:
    // 默认构造函数（空数组）
    constexpr SizeArrayRef() noexcept : data_(nullptr), size_(0) {}

    // 从 C 数组构造
    template <size_t N>
    constexpr SizeArrayRef(const size_t (&arr)[N]) noexcept : data_(arr), size_(N)
    {}

    // 从指针和大小构造
    constexpr SizeArrayRef(const size_t *data, size_t size) noexcept : data_(data), size_(size) {}

    // 从 std::vector<size_t> 构造（零拷贝）
    SizeArrayRef(const std::vector<size_t> &vec) noexcept : data_(vec.data()), size_(vec.size()) {}
...
#endif
    SizeArrayRef(std::initializer_list<size_t> list) noexcept : data_(list.begin()), size_(list.size()) {}
#if defined(__GNUC__) || defined(__clang__)
#    pragma GCC diagnostic pop
    ...
    // 访问接口
    constexpr const size_t *data() const noexcept
    {
        return data_;
    }
    ...
    // 迭代器接口
    constexpr const size_t *begin() const noexcept
    {
        return data_;
    }
    constexpr const size_t *end() const noexcept
    {
        return data_ + size_;
    }
    constexpr const size_t *cbegin() const noexcept
    {
        return data_;
    }
    constexpr const size_t *cend() const noexcept
    {
        return data_ + size_;
    }
