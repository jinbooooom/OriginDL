...
Tensor Flatten::forward(const Tensor &input)
{
    ...
    {
        THROW_INVALID_ARG("Flatten: start_dim {} is out of range for input with {} dimensions", 
                         actual_start_dim, ndim);
    }
    if (actual_end_dim < 0 || actual_end_dim >= ndim)
    {
        THROW_INVALID_ARG("Flatten: end_dim {} is out of range for input with {} dimensions", 
                         actual_end_dim, ndim);
    }
    if (actual_start_dim > actual_end_dim)
    {
        THROW_INVALID_ARG("Flatten: start_dim {} must be <= end_dim {}", 
                         actual_start_dim, actual_end_dim);
    }
