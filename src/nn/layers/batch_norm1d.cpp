...
Tensor BatchNorm1d::forward(const Tensor &input)
{
    ...
    {
        THROW_RUNTIME_ERROR("BatchNorm1d forward: input must be 2D (N, C), but got shape {}", 
                           input_shape.to_string());
    }

    if (input_shape[1] != static_cast<size_t>(num_features_))
    {
        THROW_RUNTIME_ERROR("BatchNorm1d forward: input feature size {} does not match num_features {}", 
