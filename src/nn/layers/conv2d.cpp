...
Conv2d::Conv2d(int in_channels, int out_channels, std::pair<int, int> kernel_size,
    ...
    : in_channels_(in_channels), out_channels_(out_channels), kernel_size_(kernel_size),
      stride_(stride), pad_(pad), use_bias_(bias)
{
    // 验证参数有效性
    if (in_channels <= 0)
    {
    ...
    {
        THROW_INVALID_ARG("Conv2d: kernel_size must be positive, but got ({}, {})", 
                         kernel_size.first, kernel_size.second);
    }
    if (stride.first <= 0 || stride.second <= 0)
    {
        THROW_INVALID_ARG("Conv2d: stride must be positive, but got ({}, {})", 
                         stride.first, stride.second);
    }
    if (pad.first < 0 || pad.second < 0)
    {
        THROW_INVALID_ARG("Conv2d: pad must be non-negative, but got ({}, {})", 
...
Parameter Conv2d::init_weight()
{
    ...
    auto weight_tensor = Tensor::randn(
        Shape{static_cast<size_t>(out_channels_), static_cast<size_t>(in_channels_),
    ...
    {
        THROW_RUNTIME_ERROR("Conv2d init_weight: Parameter w has empty shape after construction! scaled_weight.shape() = {}",
...
Parameter Conv2d::init_bias()
{
    if (use_bias_)
    {
        ...
        auto bias_tensor =
            Tensor::zeros(Shape{static_cast<size_t>(out_channels_)}, TensorOptions(DataType::kFloat32));
...
Tensor Conv2d::forward(const Tensor &input)
{
    ...
    {
        THROW_RUNTIME_ERROR("Conv2d forward: input must be 4D (N, C, H, W), but got shape {}", 
    ...
    {
        THROW_RUNTIME_ERROR("Conv2d forward: input channel mismatch - input has {} channels, but layer expects {} channels",
