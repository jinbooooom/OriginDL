...
std::shared_ptr<Operator> OperatorMapper::create_yolo_detect(std::shared_ptr<PNNXNode> node)
{
    ...
    for (int32_t i = 0; i < stages; ++i)
    {
        ...
        if (i == 0)
        {
            ...
            {
                THROW_RUNTIME_ERROR("YoloDetect: conv weight must be 4D, but got shape {}", weight_shape.to_string());
            }
    ...
    return std::make_shared<functional::YoloDetect>(stages, num_classes, num_anchors, std::move(strides),
                                                    std::move(anchor_grids), std::move(grids), std::move(conv_weights),
