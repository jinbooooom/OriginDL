...
std::vector<Tensor> FlattenOp::forward(const std::vector<Tensor> &xs)
{
    ...
    if (start_dim < 0 || start_dim >= static_cast<int>(x_dims.size()) ||
        end_dim < 0 || end_dim >= static_cast<int>(x_dims.size()) ||
        start_dim > end_dim)
    {
        THROW_INVALID_ARG("Flatten: invalid dims - start_dim={}, end_dim={}, shape={}", 
                         start_dim_, end_dim_, x_shape.to_string());
    }
