...
std::vector<Tensor> AvgPool2d::forward(const std::vector<Tensor> &xs)
{
    ...
    if (x_shape.size() != 4)
    {
        THROW_RUNTIME_ERROR("AvgPool2d forward: x must be 4D (N, C, H, W), but got shape {}", x_shape.to_string());
    }

    // 获取 Mat 引用并调用底层 avg_pool2d
...
std::vector<Tensor> AvgPool2d::backward(const std::vector<Tensor> &gys)
{
    if (gys.size() != 1)
    {
        THROW_RUNTIME_ERROR("AvgPool2d backward requires exactly 1 gradient, but got {}", gys.size());
    }
...
Tensor avg_pool2d(const Tensor &x, int kernel_size, int stride, int pad)
{
    return avg_pool2d(x, std::make_pair(kernel_size, kernel_size), std::make_pair(stride, stride),
