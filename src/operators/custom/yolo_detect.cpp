      ...
      conv_biases_(std::move(conv_biases))
{
    ...
    {
        THROW_RUNTIME_ERROR("YoloDetect: strides size ({}) does not match stages ({})", 
                           strides_.size(), stages_);
    }
    if (anchor_grids_.size() != static_cast<size_t>(stages_))
    {
        THROW_RUNTIME_ERROR("YoloDetect: anchor_grids size ({}) does not match stages ({})", 
                           anchor_grids_.size(), stages_);
    }
    if (grids_.size() != static_cast<size_t>(stages_))
    {
        THROW_RUNTIME_ERROR("YoloDetect: grids size ({}) does not match stages ({})", 
                           grids_.size(), stages_);
    }
    if (conv_weights_.size() != static_cast<size_t>(stages_))
    {
        THROW_RUNTIME_ERROR("YoloDetect: conv_weights size ({}) does not match stages ({})", 
                           conv_weights_.size(), stages_);
    }
    if (conv_biases_.size() != static_cast<size_t>(stages_))
    {
        THROW_RUNTIME_ERROR("YoloDetect: conv_biases size ({}) does not match stages ({})", 
                           conv_biases_.size(), stages_);
    }
}
...
std::vector<Tensor> YoloDetect::forward(const std::vector<Tensor> &xs)
{
    ...
    {
        THROW_RUNTIME_ERROR("YoloDetect forward: expected {} inputs (stages), but got {}", 
                           stages_, xs.size());
    }
    ...
    for (int32_t stage = 0; stage < stages_; ++stage)
    {
        ...
        {
            THROW_RUNTIME_ERROR("YoloDetect forward: input {} must be 4D (N, C, H, W), but got shape {}", 
                               stage, input_shape.to_string());
        }
        ...
        Shape reshape_shape{batch_size, static_cast<size_t>(num_anchors_), 
                           static_cast<size_t>(classes_info), H, W};
        ...
        for (size_t b = 0; b < batch_size; ++b)
        {
            for (size_t i = 0; i < num_boxes; ++i)
            {
                ...
                //     logd("grid_shape: {}, anchor_grid_shape: {}", grid_shape.to_string(), anchor_grid_shape.to_string());
                //     logd("anchor_w={}, anchor_h={}, stride={}", anchor_w, anchor_h, stride);
                //     logd("Before transform: sigmoid_x={}, sigmoid_y={}, sigmoid_w={}, sigmoid_h={}", 
        ...
        Tensor transformed_tensor(sigmoid_data, target_shape, 
                                  dtype(DataType::kFloat32).device(input.device()));
