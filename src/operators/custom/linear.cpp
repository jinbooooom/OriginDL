...
std::vector<Tensor> LinearOp::forward(const std::vector<Tensor> &xs)
{
    ...
    const Mat &weight_mat = mat(weight);
    ...
    // 结果: (N, out_features)
    // 通过 Mat 层调用 matmul
    const Mat &x_flat_mat   = mat(x_flat);
    const Mat &weight_t_mat = mat(weight_t);
    ...
    if (bias != nullptr)
    {
        ...
        // 通过 Mat 层调用加法
        const Mat &y_mat              = mat(y);
        const Mat &bias_broadcast_mat = mat(bias_broadcast);
...
{
    // 当前 custom Linear 的 backward 实现（暂时不实现，因为主要用于推理）
    THROW_RUNTIME_ERROR("LinearOp backward is not implemented");
