...
std::vector<Tensor> BatchNorm::forward(const std::vector<Tensor> &xs)
{
        ...
        if (xs.size() != 5)
        {
            THROW_RUNTIME_ERROR("BatchNorm operator requires exactly 5 inputs (x, gamma, beta, running_mean, running_var), but got {}", xs.size());
        }
...
std::vector<Tensor> BatchNorm::backward(const std::vector<Tensor> &gys)
    {
        ...
        for (size_t c = 0; c < num_channels; ++c)
        {
            ...
            if (num_dims_ == 2)
            {
                ...
                {
                    size_t idx = n * num_channels + c;
                    float gx_val = (gamma_data[c] / std_val) * (gy_data[idx] - mean_gy - saved_x_norm_data[idx] * mean_gy_xnorm);
                    gx_data[idx] = gx_val;
                }
            }
            else if (num_dims_ == 4)
