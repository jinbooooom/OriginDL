...
Device parse_device_string(const std::string& device_str) {
    ...
    if (trimmed.substr(0, 4) == "cuda") {
        if (trimmed == "cuda") {
            ...
            // 检查索引部分是否为空
            if (index_str.empty()) {
                THROW_INVALID_ARG("Invalid CUDA device string: '{}'. Expected format: 'cuda:0', 'cuda:1', etc. Got: '{}' (missing device index after ':')", 
            ...
            try {
                ...
                THROW_INVALID_ARG("CUDA device index out of range: '{}'. Expected integer in range [0, {}]", 
                                 index_str, INT_MAX);
            }
        }
        else {
            // 类似 'cudax:0' 的错误
            THROW_INVALID_ARG("Invalid device type: '{}'. Expected one of: cpu, cuda, cuda:0, cuda:1, etc. Got: '{}' (unknown device type)", 
                             trimmed, device_str);
        }
    }
    ...
    error_msg += " or '" + SUPPORTED_DEVICE_TYPES[0] + ":0', '" + SUPPORTED_DEVICE_TYPES[1] + ":1', etc. Got: '" + device_str + "'";
