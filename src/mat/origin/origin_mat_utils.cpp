...
Shape compute_broadcast_shape(const OriginMat &a, const OriginMat &b)
{
    ...
    // 如果一个是真正的标量（shape{}），另一个是单元素张量或标量，返回另一个的形状
    if (a_is_scalar && (b_is_single_element || b_is_scalar))
    {
        return shape_b;
    }
    if (b_is_scalar && (a_is_single_element || a_is_scalar))
    {
        return shape_a;
    }
    ...
    if (b_is_single_element && !a_is_single_element)
    {
        return shape_a;
    }
