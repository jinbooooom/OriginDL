...
void dispatch_negate(DataType dtype, const void *a, void *c, size_t n, cudaStream_t stream = 0)
{
    switch (dtype)
    {
        case DataType::kFloat32:
            launch_negate_kernel<float>(static_cast<const float *>(a), static_cast<float *>(c), n, stream);
            break;
        case DataType::kFloat64:
            launch_negate_kernel<double>(static_cast<const double *>(a), static_cast<double *>(c), n, stream);
            break;
        case DataType::kInt32:
            launch_negate_kernel<int32_t>(static_cast<const int32_t *>(a), static_cast<int32_t *>(c), n, stream);
            break;
        case DataType::kInt8:
            launch_negate_kernel<int8_t>(static_cast<const int8_t *>(a), static_cast<int8_t *>(c), n, stream);
            break;
        default:
            THROW_INVALID_ARG("Unsupported data type for CUDA negate operation: {}", dtype_to_string(dtype));
    }
}
