...
void multiply_inplace(OriginMat &a, const OriginMat &b)
{
    ...
    if (a.shape() != b.shape())
    {
        THROW_INVALID_ARG("multiply_inplace: shapes must match. a.shape() = {}, b.shape() = {}", a.shape().to_string(),
                          b.shape().to_string());
    }
    ...
    device_common::TypeDispatcher::dispatch_void(a.dtype(), [&]<typename T>() {
        launch_elementwise_kernel<T, MultiplyOp>(static_cast<const T *>(a_data), static_cast<const T *>(b_data),
                                                 static_cast<T *>(a_data), a.elements(), MultiplyOp{}, 0);
    });
