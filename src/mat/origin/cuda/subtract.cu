...
void dispatch_subtract(DataType dtype, const void *a, const void *b, void *c, size_t n, cudaStream_t stream = 0)
{
    switch (dtype)
    {
        case DataType::kFloat32:
            launch_subtract_kernel<float>(static_cast<const float *>(a), static_cast<const float *>(b),
                                          static_cast<float *>(c), n, stream);
            break;
        case DataType::kFloat64:
            launch_subtract_kernel<double>(static_cast<const double *>(a), static_cast<const double *>(b),
                                           static_cast<double *>(c), n, stream);
            break;
        case DataType::kInt32:
            launch_subtract_kernel<int32_t>(static_cast<const int32_t *>(a), static_cast<const int32_t *>(b),
                                            static_cast<int32_t *>(c), n, stream);
            break;
        case DataType::kInt8:
            launch_subtract_kernel<int8_t>(static_cast<const int8_t *>(a), static_cast<const int8_t *>(b),
                                           static_cast<int8_t *>(c), n, stream);
            break;
        default:
            THROW_INVALID_ARG("Unsupported data type for CUDA subtract operation: {}", dtype_to_string(dtype));
    }
}
                                        ...
                                        cudaStream_t stream = 0)
{
    ...
    {
        case DataType::kFloat32:
            scalar_broadcast_kernel<float, SubtractOp>
                <<<grid, block, 0, stream>>>(static_cast<const float *>(a), static_cast<const float *>(b),
                                             static_cast<float *>(c), a_elements, b_elements, c_elements, SubtractOp{});
            break;
        case DataType::kFloat64:
            scalar_broadcast_kernel<double, SubtractOp><<<grid, block, 0, stream>>>(
                static_cast<const double *>(a), static_cast<const double *>(b), static_cast<double *>(c), a_elements,
                b_elements, c_elements, SubtractOp{});
            break;
        case DataType::kInt32:
            scalar_broadcast_kernel<int32_t, SubtractOp><<<grid, block, 0, stream>>>(
                static_cast<const int32_t *>(a), static_cast<const int32_t *>(b), static_cast<int32_t *>(c), a_elements,
                b_elements, c_elements, SubtractOp{});
            break;
        case DataType::kInt8:
            scalar_broadcast_kernel<int8_t, SubtractOp><<<grid, block, 0, stream>>>(
                static_cast<const int8_t *>(a), static_cast<const int8_t *>(b), static_cast<int8_t *>(c), a_elements,
                b_elements, c_elements, SubtractOp{});
            break;
        default:
            THROW_INVALID_ARG("Unsupported data type for CUDA scalar broadcast subtract operation: {}",
