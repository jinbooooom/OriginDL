...
std::unique_ptr<Mat> embedding(const OriginMat &x, const OriginMat &vocab)
{
    ...
    if (vocab.dtype() == DataType::kFloat32 && embedding_dim % 4 == 0)
    {
        ...
        embedding_float4_kernel<<<num_blocks, threads_per_block>>>(static_cast<const int32_t *>(indices_data),
                                                                  static_cast<const float *>(vocab_data),
    ...
    else
    {
        ...
        device_common::TypeDispatcher::dispatch_void(vocab.dtype(), [&]<typename T>() {
            embedding_kernel<T><<<num_blocks, threads_per_block>>>(static_cast<const int32_t *>(indices_data),
                                                                  static_cast<const T *>(vocab_data),
...
std::unique_ptr<Mat> embedding_backward(const OriginMat &gy, const OriginMat &x, int vocab_size, int embedding_dim)
{
    ...
    if (gy.dtype() == DataType::kFloat32)
    {
        embedding_backward_kernel_float<<<num_blocks, threads_per_block>>>(
            static_cast<const float *>(grad_output_data),
            static_cast<const int32_t *>(indices_data),
            static_cast<float *>(grad_weight_data),
            num_indices,
            embedding_dim);
    }
    else if (gy.dtype() == DataType::kFloat64)
    {
        embedding_backward_kernel_double<<<num_blocks, threads_per_block>>>(
            static_cast<const double *>(grad_output_data),
            static_cast<const int32_t *>(indices_data),
            static_cast<double *>(grad_weight_data),
            num_indices,
            embedding_dim);
    }
    else
