...
std::unique_ptr<Mat> exp(const OriginMat &mat)
{
    ...
    {
        THROW_INVALID_ARG("Device mismatch in CUDA exp: expected CUDA device, got {}", mat.device().to_string());
    }
    ...
    device_common::TypeDispatcher::dispatch_void(mat.dtype(), [&]<typename T>() {
        launch_unary_kernel<T, ExpOp>(static_cast<const T *>(a_data), 
                                     static_cast<T *>(c_data), 
                                     mat.elements(), ExpOp{}, 0);
    });
