 ...
 */
template <typename T>
__global__ void reorder_pool_grad_kernel(const T *__restrict__ src, T *__restrict__ dst, size_t N, size_t C, size_t OH,
                                         size_t OW, int KH, int KW)
{
                                ...
                                std::pair<int, int> pad)
{
    ...
    auto divisor_tensor = OriginMat::from_scalar(Scalar(divisor), Shape{1}, TensorOptions(x.dtype()).device(x.device()));
                                         ...
                                         std::pair<int, int> stride, std::pair<int, int> pad)
{
    ...
    // 2. 除以 KH * KW（平均值的反向）
    float divisor = static_cast<float>(KH * KW);
    auto divisor_tensor = OriginMat::from_scalar(Scalar(divisor), Shape{1}, TensorOptions(gy.dtype()).device(gy.device()));
