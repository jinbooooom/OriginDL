                                                 ...
                                                 float stride)
{
    ...
    size_t anchor_grid_base = anchor_idx * anchor_H * anchor_W * 2 + anchor_h_idx * anchor_W * 2 + anchor_w_idx * 2;
                                         ...
                                         int32_t num_classes)
{
    ...
    {
        THROW_INVALID_ARG("yolo_detect_forward: input must be 4D (N, C, H, W), but got shape {}",
    ...
    yolo_coordinate_transform_kernel<<<grid_size, block_size>>>(sigmoid_data, result_data, grid_data, anchor_grid_data,
                                                                batch_size, num_boxes, classes_info, H, W, num_anchors,
                                                                anchor_H, anchor_W, stride);
