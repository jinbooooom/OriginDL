...
std::unique_ptr<Mat> divide(const OriginMat &a, const OriginMat &b)
{
    ...
    {
        THROW_INVALID_ARG("Data type mismatch in CPU divide: {} vs {}", 
                          dtype_to_string(a.dtype()), dtype_to_string(b.dtype()));
    }

    if (unlikely(a.device() != b.device()))
    {
        THROW_INVALID_ARG("Device mismatch in CPU divide: {} vs {}", 
                          a.device().to_string(), b.device().to_string());
    }

    if (unlikely(a.device().type() != DeviceType::kCPU))
    {
        THROW_INVALID_ARG("Device mismatch in CPU divide: expected CPU device, got {}", 
                          a.device().to_string());
    }
    ...
    if (a.shape() == b.shape())
    {
        ...
        device_common::TypeDispatcher::dispatch_void(a.dtype(), [&]<typename T>() {
            cpu_elementwise_kernel<T, DivideOp>(static_cast<const T *>(a_data), 
    ...
    else if (a.elements() == 1 || b.elements() == 1)
    {
        // 简单广播：标量广播
        device_common::TypeDispatcher::dispatch_void(a.dtype(), [&]<typename T>() {
            cpu_simple_broadcast_kernel<T, DivideOp>(static_cast<const T *>(a_data), 
                                                    static_cast<const T *>(b_data), 
                                                    static_cast<T *>(c_data), 
