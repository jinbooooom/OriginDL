...
std::unique_ptr<OriginMat> divide(const OriginMat &a, const OriginMat &b)
{
    ...
    if (a.dtype() != b.dtype())
    {
        THROW_INVALID_ARG("Data type mismatch for division: expected {} but got {}", dtype_to_string(a.dtype()),
    ...
    else
    {
        ...
        {
            THROW_INVALID_ARG(
                "Shape mismatch for division - only scalar broadcasting supported. Shape A: {}, Shape B: {}",
    ...
    switch (a.dtype())
    {
        ...
        default:
            THROW_INVALID_ARG("Unsupported data type {} for division operation", dtype_to_string(a.dtype()));
    }
