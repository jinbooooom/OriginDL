...
std::unique_ptr<Mat> subtract(const OriginMat &a, const OriginMat &b)
{
    ...
    if (a.shape() == b.shape())
    {
        ...
        device_common::TypeDispatcher::dispatch_void(a.dtype(), [&]<typename T>() {
            cpu_elementwise_kernel<T, SubtractOp>(static_cast<const T *>(a_data), 
                                                 static_cast<const T *>(b_data), 
                                                 static_cast<T *>(c_data), 
                                                 a.elements(), SubtractOp{});
        });
    }
    else if (a.elements() == 1 || b.elements() == 1)
    {
        // 简单广播：标量广播
        device_common::TypeDispatcher::dispatch_void(a.dtype(), [&]<typename T>() {
            cpu_simple_broadcast_kernel<T, SubtractOp>(static_cast<const T *>(a_data), 
                                                      static_cast<const T *>(b_data), 
                                                      static_cast<T *>(c_data), 
