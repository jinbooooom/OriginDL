...
std::unique_ptr<Mat> leaky_relu(const OriginMat &mat, const OriginMat &alpha, OriginMat *out)
{
    ...
    if (out != nullptr)
    {
        ...
        {
            THROW_INVALID_ARG(
                "Output tensor mismatch. Expected alphe.elements() = 1 , shape={}, dtype={}, device={}, but got "
    ...
    device_common::TypeDispatcher::dispatch_void(mat.dtype(), [&]<typename T>() {
        cpu_simple_broadcast_kernel<T, LeakkyReLUOP>(static_cast<const T *>(a_data), static_cast<const T *>(b_data),
                                                     static_cast<T *>(c_data), mat.elements(), alpha.elements(),
                                                     result_ptr->elements(), LeakkyReLUOP{});
    });
