...
std::unique_ptr<Mat> exp(const OriginMat &mat)
{
    ...
    device_common::TypeDispatcher::dispatch_void(mat.dtype(), [&]<typename T>() {
        cpu_unary_kernel<T, ExpOp>(static_cast<const T *>(a_data), static_cast<T *>(c_data), mat.elements(), ExpOp{});
    });
...
void exp_inplace(OriginMat &mat)
{
    ...
    // 使用类型分发器执行原地指数运算
    device_common::TypeDispatcher::dispatch_void(mat.dtype(), [&]<typename T>() {
        cpu_unary_kernel<T, ExpOp>(static_cast<const T *>(a_data), static_cast<T *>(a_data), mat.elements(), ExpOp{});
    });
}
